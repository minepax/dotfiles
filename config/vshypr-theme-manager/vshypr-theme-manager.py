...
def build_context(colors: dict) -> dict:
    ...
    rgba_variants: dict[str, list[float]] = {
        ...
        "teal": [0.10, 0.20, 0.35],
        "bg": [0.40, 0.50, 0.70, 0.80, 0.90, 0.93, 0.95, 0.96, 0.98],
...
def apply_kitty(ctx: dict):
    ...
    if kitty_conf.exists():
        ...
        kitty_conf.write_text(
            "# Kitty config — generado por theme-changer\ninclude theme.conf\n"
        )
...
def apply_waybar(ctx: dict):
    ...
    if vswaybar_managed:
        ...
        missing = [
            f"@define-color {n} {v};" for n, v in tokens.items() if n not in updated
        ]
        if missing:
            content = "\n".join(missing) + "\n" + content
...
def apply_eww(ctx: dict, variant: str = "dark"):
    ...
    if result.returncode != 0:
        subprocess.Popen(
            ["eww", "daemon"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
def apply_gtk4(ctx: dict):
    ...
    if GTK4_CSS.is_symlink():
        GTK4_CSS.unlink()
        GTK4_CSS.write_text(
            "/* GTK4 color overrides — gestionado por theme-changer */\n"
        )
...
@define-color warning_color               {ctx["yellow"]};"""
    ...
    if GTK4_DARK_CSS.is_symlink():
        ...
        GTK4_DARK_CSS.write_text(
            "/* GTK4 dark overrides — gestionado por theme-changer */\n"
        )
...
def apply_gtk3(ctx: dict):
    ...
    if not GTK3_CSS.exists():
        GTK3_CSS.write_text(
            "/* GTK3 color overrides — gestionado por theme-changer */\n"
        )
...
"""
    ...
    if QT6CT_CONF.exists():
        ...
        text = re.sub(
            ...
            f"color_scheme_path={QT_COLOR_SCHEME}",
            text,
        )
...
def apply_qt5ct(ctx: dict):
    ...
    if QT5CT_CONF.exists():
        ...
        text = re.sub(r"(?m)^custom_palette\s*=.*$", "custom_palette=true", text)
        text = re.sub(
            r"(?m)^color_scheme_path\s*=.*$", f"color_scheme_path={QT5CT_COLORS}", text
        )
        if "color_scheme_path" not in text:
            text = text.replace(
                "[Appearance]",
    ...
    for rc_file in list(CFG.glob("*.rc")) + list(CFG.glob("*rc")):
        try:
            ...
            if "ColorScheme=" in rc_text:
                rc_text = re.sub(
                    r"(?m)^ColorScheme\s*=.*$", "ColorScheme=ThemeChanger", rc_text
                )
...
def apply_kvantum(ctx: dict):
    ...
    if base_kv.exists():
        ...
        base_content = re.sub(
            r"\[GeneralColors\].*?(?=\[|\Z)", "", base_content, flags=re.DOTALL
        )
    else:
...
def apply_wallpaper_sh(wallpaper: str):
    ...
    new_content = re.sub(r'(awww img\s+")[^"]*(")', rf"\g<1>{wallpaper}\g<2>", content)
    if new_content != content:
...
def save_state(theme: str, wallpaper: str | None = None):
    STATE_FILE.write_text(
        json.dumps({"theme": theme, "wallpaper": wallpaper}, indent=2)
    )
...
def apply_theme(theme_name: str, wallpaper: str | None = None):
    ...
    nautilus_running = (
        subprocess.run(["pgrep", "-x", "nautilus"], capture_output=True).returncode == 0
    )
    if nautilus_running:
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "  vshypr-theme-manager.py <tema> <wallpaper>      Aplica tema + wallpaper"
        )
        print(
            "  vshypr-theme-manager.py dynamic-dark  <wallpaper>  Genera paleta oscura con matugen"
        )
        print(
            "  vshypr-theme-manager.py dynamic-light <wallpaper>  Genera paleta clara con matugen"
        )
        print(
            "  vshypr-theme-manager.py --list                  Lista temas disponibles"
        )
        sys.exit(0)
