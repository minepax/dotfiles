...
stream_entries | rofi \
    ...
    -theme /home/mahesh/.config/rofi/config.rasi \
    >/tmp/rofi-theme-sel
ROFI_EXIT=$?
...
if [[ "$SELECTED_THEME" == "dynamic-dark" || "$SELECTED_THEME" == "dynamic-light" ]]; then
    ...
    mapfile -d '' WALLPAPERS < <(find "/home/mahesh/Pictures/Wallpapers" \
    ...
    WP_IDX=$(for wp in "${WALLPAPERS[@]}"; do
        ...
        -p "󰋩  wallpaper" \
        -format "i" \
        -theme /home/mahesh/.config/rofi/wallpaper-picker.rasi)
